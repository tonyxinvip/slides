(() => {
  ...
  function showSlide(index, updateHash = true) {
    ...
    document.title = `${pad(current + 1)} · ${slides[current].dataset.title} — AI Worker`;
    updateNotes();
  ...
  function buildOverview() {
    ...
    slides.forEach((slide, index) => {
      ...
      button.addEventListener('click', () => { showSlide(index); toggleOverview(false); });
  ...
  document.addEventListener('keydown', event => {
    ...
    else if (['ArrowLeft', 'PageUp'].includes(event.key)) { event.preventDefault(); showSlide(current - 1); }
  ...
  deck.addEventListener('touchend', event => {
    ...
    if (Math.abs(deltaX) > 55 && Math.abs(deltaX) > Math.abs(deltaY) * 1.25) showSlide(current + (deltaX < 0 ? 1 : -1));
  }, { passive: true });
