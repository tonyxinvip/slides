...
/* 各段时长是编排判断，写在这里，不从数据推。合计须为 90。 */
const MINUTES = { '开场':8, '第一段':10, '第二段':8, '第三段':16, '第四段':12, '第五段':20, '第六段':10, '第七段':6 };
const SECTITLE = {
  '开场':'领航行动指南与本课程的关系',
  '第一段':'做中学对综合实践活动的覆盖范围',
  '第二段':'成果可替代性与教师的专业位置',
...
const BLOCKS = {
  meta: [
    '版本：90 分钟版，' + SLIDES.length + ' 页。每页标注删减档位，砍到 60 分钟时按档位删。'
  ].join('\n'),
  ...
  minutes: [
    ...
    '| ' + NUM[s] + ' | ' + MINUTES[s] + ' 分钟 | ' + bySec(s).length + ' | ' + SECTITLE[s] + ' |'
  )).concat(['| 合计 | ' + mins + ' 分钟 | ' + SLIDES.length + ' | |']).join('\n'),
  ...
  pages: secs.map(s => {
    ...
    return ['### ' + s + '（' + MINUTES[s] + ' 分钟，' + bySec(s).length + ' 页）', '',
            '| 页 | 档 | 标题 | 屏幕上的元素 | 讲稿落点 |',
            '| --- | --- | --- | --- | --- |'].concat(rows).join('\n');
  }).join('\n\n'),
  ...
  deliver: '1. `做中学与综合实践活动.pptx`，' + SLIDES.length + ' 页，每页配 speaker notes'
};
...
secs.forEach(s => {
  ...
  data = data.replace(re, '  /* ' + '═'.repeat(10) + ' ' + BANNER[s] + '（' + MINUTES[s] +
    ' 分钟，' + bySec(s).length + ' 页）' + '═'.repeat(10) + ' */\n$1');
});
data = data.replace(/\n( {2}\/\* ═)/g, '\n\n$1').replace(/\n{3,}( {2}\/\* ═)/g, '\n\n$1');
...
console.log('OUTLINE.md 已重写：' + SLIDES.length + ' 页，' + mins + ' 分钟，A ' +
  tiers('A').length + ' / B ' + tiers('B').length);
