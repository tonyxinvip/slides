(() => {
  ...
  function updateNotes() {
    notesContent.textContent = slides[current].querySelector('.speaker-note')?.textContent.trim() || '本頁沒有備註。';
  }
  ...
  document.addEventListener('keydown', event => {
    if (help.open || /INPUT|TEXTAREA|SELECT/.test(event.target.tagName)) return;
  ...
  document.addEventListener('touchend', e => { if (!overview.hidden || !notes.hidden || help.open) return; const dx=e.changedTouches[0].clientX-touchX; const dy=e.changedTouches[0].clientY-touchY; if(Math.abs(dx)>55&&Math.abs(dx)>Math.abs(dy)*1.25) show(current+(dx<0?1:-1)); }, {passive:true});
